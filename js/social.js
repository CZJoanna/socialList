...
function renderUserList(array) {
    ...
    array.forEach((data) => {
        ...
        dataCard += `
         <div class="friendCard" id="userCard">
            <div>
                <img src="${image}">
                <p>${name}&nbsp;${surName}</p>
            </div>
            <div>
                ...
                >VISIT !</button>
                <button class="btn-add-favorite favBtn" data-id="${id}">+</button>
            </div>
...
function getInputValue() {
    ...
    if (inputVal.length) {
        ...
        if (filterArray.length) {
            const total_filter_pages = Math.ceil(
                filterArray.length / users_per_page
            );
